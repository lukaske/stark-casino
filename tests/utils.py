...
class Transmitter():
    ...
    async def send_transaction(self, account, to, selector_name, calldata, nonce=None):
        rrc, robs,  obs, r_sigs, s_sigs, r_signers = calldata
        if nonce is None:
            execution_info = await account.get_nonce().call()
            nonce, = execution_info.result
        ...
        return await account.transmit(to, selector, rrc, robs,  obs, r_sigs, s_sigs, r_signers, nonce).invoke(signature=[sig_r, sig_s])
...
def transmitter_hash_message(sender, to, selector, calldata, nonce):
    ...
    calldata_hash = compute_hash_on_elements(
        [rrc, robs] + obs + r_sigs + s_sigs + [r_signers])
